...
class ProductForm(forms.ModelForm):
    try:
        ...
        for choice in d:
            ls = []
            c_tup = tuple([choice.name[0:3], choice.name])
            ls.append(c_tup)
            CategoryChoices += tuple(ls)

    except Exception as e:
            pass
    category = forms.CharField(widget=forms.Select(choices=CategoryChoices))

    class Meta:
        model = Product
        fields = ['name', 'sortno', 'category']
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control', 'id': 'name'}),
